...
from campaigns.yiquan import const, models
...
class VoteCheatProcess(cheat.CheatProcess):
    def __init__(self, vote_cheat):
        ...
        if self.vote_cheat.type == const.VoteCheatConst.TYPE_ONE:
            ...
            total_id_list = models.YQWork.objects.values_list('id', flat=True).filter(status=FoundationConst.FIX)
    ...
    def cheat(self):
        work = models.YQWork.objects.get(pk=self.work_id_list[self.now_count])
