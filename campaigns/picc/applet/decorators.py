...
from campaigns.picc.applet.uitls import generate_other_dict_data
from campaigns.picc import app_id, models
from django.utils.http import urlquote
from django.http import HttpResponseRedirect, HttpResponse, Http404, HttpResponseServerError
from campaigns.picc import wechat_api
...
def _record_uv(request):
    ...
    suchUv = models.UniqueVisitor.objects.filter(wxUser__openid=openid).filter(creationTime__gte=nowTime).first()
    if suchUv is None:
